...
core <- function(x0, y0, target, rstart, tar.lab.col = "black", tar.lab.cex = 1.1, tar.lab.font = 1, core.line.col = 'black'){
  text(x0, y0, labels = target, adj = c(0.5, 0.5), cex = tar.lab.cex, col = tar.lab.col, font = tar.lab.font);
...
ring <- function(x0, y0, layers, rstart, flat, ring.text.col = "black", ring.text.cex = 1.0, ring.text.font = 1, ring.line.col = "gray"){
  for (val in layers){
    text(x0, y0 + val * flat + rstart, labels = val, adj = c(0.5, 1.2), col = ring.text.col, cex = ring.text.cex, font = ring.text.font)  #rstart = rmin: mininum radius
...
plot_values <- function(x0, y0, data, amax, amin, rstart, label_mode, pop.lab.cex = 0.7, pop.lab.font = 1, pop.lab.col = "navy"){
  ...
  for (i in 1:nrow(data)){
    ...
    if(label_mode){
      if(pmean[3] < -90){
         text(x0 + pmean[1], y0 + pmean[2], data[i, pop_col], srt = 180 + pmean[3], adj = c(1.1, 0.5), cex = pop.lab.cex, font = pop.lab.font, col = pop.lab.col)
        }else{
         text(x0 + pmean[1], y0 + pmean[2], data[i, pop_col], srt = pmean[3], adj = c(-0.1, 0.5), cex = pop.lab.cex, font = pop.lab.font, col = pop.lab.col)
      }
    }
    angpre <- ang2
  }
...
plot_legend <- function(data, legend.pos = "topright", legend.lwd = 5, legend.text.cex = 1.00){
  legend(legend.pos, legend = as.character(data[, 1]), lwd = legend.lwd, col = data[,2], cex = legend.text.cex, bty = 'n')
}
...
#' @return NULL
#' @export
radiationplot <- function(data, target = "target", cenvals = c(0.5, 0.5), sorting = FALSE, layers = NULL, num = 4, 
                          amax = -250, amin = 70, border = 0.3, rstart = 0.02, flat = 1.2, 
  ...
  ring(x0, y0, layers, rstart, flat, ring.text.col = ring.text.col, ring.text.cex = ring.text.cex, ring.text.font = ring.text.font, ring.line.col = ring.line.col)
